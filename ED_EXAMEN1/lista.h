...
public:
    ...
    virtual void eliminarNodo(nodo<T>*);
...
template<class T>
void lista<T>::eliminarNodo(nodo<T>* n)
{
    try
    {
        nodo<T>* valorAnterior = NULL;
        ...
        {
            valorAnterior = cursor;
            cursor = cursor -> obtenerSiguiente();
        }

        if (cursor == nullptr)
        {
            throw exception("Hubo un error al borrar el nodo");
            return;
        }
        else
        {
            valorAnterior -> fijarSiguiente(cursor -> obtenerSiguiente());
            delete cursor;
        }
    }
    catch (exception& e)
